---
name: tuicr
description: Use tuicr's review CLI to read and add comments in active TUI review sessions, and launch tuicr in tmux, Zellij, Herdr, or Orca when a user needs an interactive review pane.
---
...
| `$TERM_PROGRAM` is `Orca` or `$ORCA_TERMINAL_HANDLE` is set | Run `tuicr-wrapper-orca.sh /path/to/repo` |
...
<skill-directory>/tuicr-wrapper-orca.sh /path/to/repo
```
...
| No active session, tmux/Zellij/Herdr/Orca available | Start a new tuicr pane with the matching wrapper |
