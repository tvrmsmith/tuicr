...
- Grouping: `grouping: Option<Grouping>` and `grouping_enabled` — `grouping` stays populated while grouping is toggled off, so ask `App::active_grouping()` for the grouping the sidebar is rendering, plus the refine arm's `refine_config`, `refine_target_picked`, `refine_wanted`, `refine_over_saved_grouping`, `pending_grouping` and `pending_regroup` (see `src/app/grouping.rs`, `src/app/refine.rs` and `src/app/regroup.rs`)
