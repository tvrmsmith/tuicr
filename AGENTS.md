...
├── persistence/
│   ├── mod.rs
│   ├── feedback_log.rs  # grouping-feedback.jsonl: the payload, its path, the locked append
...
- Grouping feedback (`gd-26r.42`): the verdict prompt lives in `src/app/grouping_feedback.rs` — `grouping_feedback: Option<GroupingFeedbackDraft>` (the open prompt), `grouping_feedback_settled` (submitted or skipped, so `:w`/`:send` ask once a session) and `grouping_feedback_enabled` (`[grouping].feedback`, which gates the `x` marks too). `maybe_prompt_grouping_feedback` is called from the `Write` and `Release` arms of `dispatch_command` only, never from `save_current_session_merging_external`, whose other callers include `:wq` and `ZZ`: quit never prompts. `GroupingFeedbackDraft::into_feedback` is the pure conversion, and the private `App::record_grouping_feedback` builds the log entry and appends it
- Grouping feedback (`gd-26r.43`): a submitted verdict appends one JSON line to `<data dir>/grouping-feedback.jsonl` via `persistence::feedback_log::append`, and a skip writes nothing. `GroupingFeedbackEntry::build` is pure and takes an `EntrySources` struct; the caller resolves the slug and canonicalizes the repo path, which are the only I/O in the payload. `files` is built from `grouping.assignments()`, never from `diff_files`, because the prompt also fires with `<leader>g` off and `diff_files` is then in directory order. tuicr never reads the file back: see `docs/GROUPING_FEEDBACK.md` for the schema and the rulings behind it
- The refine arm the log records travels with the grouping it produced: `pending_grouping_arm` parks a `RefineAttempt` beside `pending_grouping`, and `App::order_files_by_group` adopts both into `grouping_arm` together. `App::land` takes the arm as an explicit parameter, so the `gd-26r.36` backstop — heuristics-only, never refine — passes `None` and cannot leave a stale refine call attributed to a partition it never touched
