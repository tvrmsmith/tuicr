...
impl ReviewSession {
    ...
    pub fn add_file(&mut self, path: PathBuf, status: FileStatus, content_hash: u64) -> bool {
        if let Some(review) = self.files.get_mut(&path) {
            // Read before the overwrite below: `rehashed` needs the hash the
            // file was last seen with, which the next line destroys.
            ...
            if old_hash.is_some_and(|old| old != content_hash) {
